...
export default function PartsDiagram({parts}:{parts:Content['parts']}){
 ...
 return(<div>
  <svg viewBox="0 0 320 230" role="group" aria-label="Sewing machine diagram. Select a dot, or use the buttons below." className="w-full rounded-swatch border-2 border-maroon/20 bg-paper shadow-card">
  ...
  {cur&&<div aria-live="polite" className="card mt-3 border-l-4 border-l-mustard p-4"><h2 className="font-serif text-xl font-bold text-maroon">{cur.name}</h2><p className="mt-1">{cur.fn}</p></div>}
