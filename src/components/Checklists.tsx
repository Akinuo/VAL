...
export default function Checklists({lists}:{lists:Content['checklists']}){
 ...
 return(<div className="grid gap-6">{lists.map(l=><section key={l.slug} id={l.slug} className="card scroll-mt-4 p-4">
  <h2 className="font-serif text-2xl font-semibold text-maroon">{l.title}</h2>
  ...
  <button className="mt-1 underline underline-offset-2" onClick={()=>put(on.filter(x=>!x.startsWith(l.slug+':')))}>Clear this checklist</button></section>)}</div>)}
