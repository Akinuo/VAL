...
export default function LessonPlayer({lesson}:{lesson:Lesson}){
 ...
 return(<article>
  ...
  <div className="mt-4 grid aspect-video place-items-center rounded bg-ink text-cream">
  ...
  <ol aria-label="Steps" className="mt-5 flex gap-2">{lesson.steps.map((t,n)=><li key={t.id}><button onClick={()=>go(n)} aria-current={n===i?'step':undefined} aria-label={`Step ${n+1}${done.has(t.id)?', passed':''}`} className={`h-11 w-11 rounded-full border-2 border-maroon font-bold ${n===i?'bg-maroon text-cream':done.has(t.id)?'bg-sage':'bg-white'}`}>{done.has(t.id)?'✓':n+1}</button></li>)}</ol>
  <section className="mt-4" aria-labelledby="st">
   <h2 id="st" className="font-serif text-2xl">Step {i+1} of {lesson.steps.length}: {s.title}</h2>
   ...
   <div aria-live="polite" className="mt-3 min-h-[3.5rem]">
    {pick!==null&&q&&(ok?<p className="font-semibold text-maroon">Correct. {q.explanation}</p>:<p className="font-semibold">Not quite. Re-read the step, then <button className="underline" onClick={()=>setPick(null)}>try again</button>.</p>)}
   </div>
   <div className="flex flex-wrap gap-2">
    {i>0&&<button className="btn" onClick={()=>go(i-1)}>Previous step</button>}
    {last?<Link href="/" className="btn">Back to dashboard</Link>:<button className="btn" disabled={!(ok||done.has(s.id))} onClick={()=>go(i+1)}>Next step</button>}
   </div>
  </section></article>)}
