...
export default function RootLayout({children}:{children:React.ReactNode}){
 return(<html lang="en"><body className="bg-cream font-sans text-ink">
  ...
  <Providers><Nav/><main id="main" className="mx-auto max-w-3xl px-4 py-8">{children}</main></Providers>
  <Stitch className="mx-auto max-w-3xl px-4 text-maroon/30"/>
  <footer className="mx-auto max-w-3xl px-4 py-6 text-sm text-ink/70">VAL Guide for BTLED Home Economics students. Videos load only when you press play, so lessons stay light on slow connections.</footer>
 </body></html>)}
