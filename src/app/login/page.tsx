...
export default function Login(){
 ...
 return(<><h1 className="font-serif text-3xl font-bold text-maroon">{up?'Create an account':'Log in'}</h1>
  ...
  <form onSubmit={submit} className="card mt-4 grid max-w-md gap-4 p-5">
   ...
   <button type="button" className="min-h-[44px] justify-self-start text-sm underline underline-offset-2" onClick={()=>{setUp(!up);setMsg('')}}>{up?'I already have an account':'I need an account'}</button>
